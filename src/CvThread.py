...
class CvThread(threading.Thread):
	...
	def run(self):
		while not self._evt.isSet():
			...
			if self._q.empty(): # Wait for camera
				continue
			...
			hsv_thresh = cv2.inRange(frameROI_HSV, (0,0,0,0), (180,255,self._hsvThreshold,0)) 
			hsv_thresh = cv2.morphologyEx(hsv_thresh, cv2.MORPH_CLOSE, np.ones((2,2), np.uint8)) # Fill (small) holes
			_, hsv_thresh_contours, _ = cv2.findContours(hsv_thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE) #Detect countours
			
			try:
				bestContour = max(hsv_thresh_contours, key=lambda p: cv2.boundingRect(p)[3]) # Take the highest (yAxis) contour
			except:
				bestContour = None
			...
			cv2.line(frameBGR, (int(frameBGR.shape[1] / 2), 0), (int(frameBGR.shape[1] / 2), frameBGR.shape[0]), (0, 255, 255),2)
